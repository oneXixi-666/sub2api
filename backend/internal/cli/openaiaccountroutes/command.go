...
const Name = "openai-account-routes"

func Run(program string, args []string, output io.Writer) error {
	if program == "" {
		program = Name
	}
	if output == nil {
	...
	if len(args) == 0 {
		return fmt.Errorf("usage: %s <list|set|delete> [flags]", program)
	}
	...
	case "list":
		...
		for _, id := range ids {
			fmt.Fprintf(output, "api_key_id=%d account_id=%d\n", id, values[id])
		}
		return nil
	case "set":
		...
		fmt.Fprintf(output, "set api_key_id=%d account_id=%d\n", apiKeyID, accountID)
		return nil
	case "delete":
		if apiKeyID <= 0 {
		...
		fmt.Fprintf(output, "deleted api_key_id=%d\n", apiKeyID)
		return nil
	default:
