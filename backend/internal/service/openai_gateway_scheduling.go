...
func (s *OpenAIGatewayService) recheckSelectedOpenAIAccountFromDBForForcedRoute(ctx context.Context, account *Account, platform string, requestedModel string, requireCompact bool, requiredCapability OpenAIEndpointCapability) *Account {
	latest := s.recheckSelectedOpenAIAccountFromDBBeforeProfitWithGroupCheck(ctx, account, nil, platform, requestedModel, requireCompact, requiredCapability, false)
	if latest == nil {
		return nil
	}
	if vetoed, _ := openAIProfitControlVetoReason(ctx, latest); vetoed {
...
func (s *OpenAIGatewayService) recheckSelectedOpenAIAccountFromDBBeforeProfit(ctx context.Context, account *Account, groupID *int64, platform string, requestedModel string, requireCompact bool, requiredCapability OpenAIEndpointCapability) *Account {
	return s.recheckSelectedOpenAIAccountFromDBBeforeProfitWithGroupCheck(ctx, account, groupID, platform, requestedModel, requireCompact, requiredCapability, true)
}

func (s *OpenAIGatewayService) recheckSelectedOpenAIAccountFromDBBeforeProfitWithGroupCheck(ctx context.Context, account *Account, groupID *int64, platform string, requestedModel string, requireCompact bool, requiredCapability OpenAIEndpointCapability, checkGroup bool) *Account {
	if account == nil {
		return nil
	}
	platform = NormalizeOpenAICompatiblePlatform(platform)
