...
func ProvideSettingService(settingRepo SettingRepository, groupRepo GroupRepository, proxyRepo ProxyRepository, cfg *config.Config) *SettingService {
	...
	if err := svc.StartOpenAIAPIKeyAccountRouteReload(); err != nil {
		logger.LegacyPrintf("service.setting", "Warning: load OpenAI API key account routes failed: %v", err)
	}
	...
	if err := svc.EnsureDisplaySettings(context.Background()); err != nil {
		logger.LegacyPrintf("service.setting", "Warning: inject display locale/currency settings failed: %v", err)
	}
