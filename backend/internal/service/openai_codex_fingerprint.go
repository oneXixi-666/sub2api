...
// codexFingerprintModeFromExtra applies the account-level default used by
// GetCodexFingerprintMode. New and fully edited OpenAI OAuth accounts therefore
// receive the seed required by the default session convergence mode.
...
// explicitlyConfiguredCodexFingerprintMode reads a key-level Extra update.
// Missing mode means the update does not alter convergence and must not create
// a seed as a side effect.
func explicitlyConfiguredCodexFingerprintMode(extra map[string]any) codexFingerprintMode {
	if extra == nil {
	...
	case codexFingerprintDevice, codexFingerprintSession, codexFingerprintFull:
		return codexFingerprintMode(strings.TrimSpace(value))
	default:
		return codexFingerprintOff
	}
}
...
func ShouldEnsureCodexFingerprintSeedForExtraUpdates(updates map[string]any) bool {
	return codexFingerprintModeRequiresSeed(explicitlyConfiguredCodexFingerprintMode(updates))
}

// GetCodexFingerprintMode 从账号 extra JSON 读取指纹收敛模式。
//
// 缺失、空值或非法值默认使用 session 收敛；管理员仍可显式选择 off，或选择
// 更弱/更强的 device/full 模式。session 是共享 OAuth 账号时的平衡默认值：
// 账号级设备与会话保持稳定，真实客户端 session 仍可派生出独立 thread。
