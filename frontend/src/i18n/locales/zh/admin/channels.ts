export default {
    ...
    riskControl: {
      ...
      cyberPolicyExcludeBanHint: '开启后，封控范围内的 cyber_policy 命中不再计入自动封号次数；审核证据与处置通知仍会保留。仅收集分组本身不会计数或通知。',
      ...
      cyberEvidenceTitle: 'cyber_policy 对话证据',
      cyberEvidenceHint: '该内容用于后续人工审核，不代表上游分类器的精确触发词。',
      evidenceProtocol: '协议',
      evidenceStage: '请求阶段',
      evidenceSize: '采集规模',
      evidenceSizeValue: '{chars} 字符 / {messages} 个文本片段',
      evidenceHash: '规范化输入哈希',
      evidenceTruncated: '内容已截断',
      ...
      groupScope: '审计分组',
      groupScopeHint: '控制主动内容审核范围；不影响 cyber_policy 证据采集。',
      hardBlockGroupScope: '硬拦截分组',
      hardBlockGroupScopeHint: '仅这些分组允许本地关键词和哈希硬拦截；其他分组仍可独立观察。',
      userObservationGroupScope: '用户输入观察分组',
      userObservationGroupScopeHint: '控制真实用户输入的关键词观察和审核 API 范围。',
      contextObservationGroupScope: '上下文观察分组',
      contextObservationGroupScopeHint: '控制 assistant、tool 和 environment 内容的关键词观察范围。',
      cyberEnforcementScope: 'cyber_policy 封控分组',
      cyberEnforcementScopeHint: '命中范围内执行后续会话屏蔽、违规计数和处置通知；其他分组仅收集审核证据。',
      cyberEnforcementBoundary: '注意：分组范围只能控制本地后续处置。上游已经返回 cyber_policy 的当次请求仍会被拒绝，无法在网关侧恢复。',
      cyberPolicyEnforceMode: '执行封控',
      cyberPolicyCollectMode: '仅收集',
      cyberPolicyCollectDescription: '仅保存对话证据和上游错误，不封控会话、不累计违规、不发通知，也不会触发封号。',
      cyberDefaultPolicy: 'Cyber 默认策略',
      cyberDefaultPolicyHint: '没有单独策略的分组继承这里的设置。证据采集始终开启，策略只决定后续处置。',
      cyberPolicyInheritedBy: '{count} 个分组继承',
      cyberGroupPolicies: '分组覆盖策略',
      cyberGroupPoliciesHint: '为需要不同处置力度的分组建立完整独立策略。删除覆盖后，该分组重新继承默认策略。',
      cyberSelectGroup: '选择要单独配置的分组',
      cyberNoGroupsToAdd: '所有分组都已配置',
      cyberAddGroupPolicy: '添加策略',
      cyberNoGroupPolicies: '暂无分组覆盖策略',
      cyberNoGroupPoliciesHint: '当前所有分组使用 Cyber 默认策略。',
      cyberGroupOverrideHint: '该策略完全替代 Cyber 默认策略',
      cyberRemoveGroupPolicy: '删除分组策略',
      cyberUnknownGroup: '未知分组 #{id}',
      cyberCollectionInvariant: '所有官方 cyber_policy 命中都会进入审核记录，不受普通审计分组和模型范围限制。仅风险控制总开关关闭时停止采集。',
      cyberSessionBlock: '会话封控',
      cyberSessionBlockShortHint: '阻止该会话继续请求',
      cyberSessionBlockTTL: '会话封控时长（秒）',
      cyberSessionBlockMasterHint: '此项还受系统设置中的“Cyber 会话屏蔽”总开关控制。',
      cyberViolationCount: '累计违规',
      cyberViolationCountHint: '按账号和当前分组独立计数',
      cyberEmailNotice: '命中通知',
      cyberEmailNoticeHint: '每次命中发送邮件',
      cyberAutoBanAccount: '自动封禁账号',
      cyberAutoBanAccountHint: '达到该分组阈值后禁用整个用户账号并刷新认证缓存',
      cyberGroupCounterHint: '不同分组的 Cyber 次数互不累加。封号结果作用于整个账号。',
      cyberPolicySnapshotTitle: '命中时的生效策略',
      cyberPolicySnapshotHint: '这里保存的是命中发生时的策略快照，后续修改配置不会改变历史依据。',
      cyberPolicySourceDefault: '继承默认策略',
      cyberPolicySourceGroup: '分组覆盖策略',
      selectedGroups: '指定分组',
      selectedGroupCount: '已选择 {count} 个分组',
      searchGroups: '搜索分组名称或平台',
      noGroups: '暂无可用分组',
      modelFilter: '模型范围',
      modelFilterHint: '按客户端请求的模型名决定是否执行主动内容审计；不限制官方 cyber_policy 证据采集。',
      modelFilterAll: '所有模型',
      ...
      blockedKeywords: '用户输入观察词',
      blockedKeywordsPlaceholder: '每行一个仅用于观察真实用户输入的词',
      blockedKeywordsDescription: '只扫描当轮明确用户输入及无角色输入；命中不标记违规、不计数、不发信、不封号。',
      contextObservationKeywords: '上下文观察词',
      contextObservationKeywordsPlaceholder: '每行一个工具或模型产物观察词；建议保持少量',
      contextObservationKeywordsDescription: '只扫描 assistant、tool 和完整环境信封，不扫描固定 system/developer 提示；命中只记录观察。',
      blockedKeywordsPreBlockHint: '宽泛词只观察；只有高置信度硬拦截词和上游审计结果可以阻断请求。',
      blockedKeywordsModeWarning: '当前为「{mode}」模式；观察词仍只记录，高置信度词不会执行本地硬拦截。',
      blockedKeywordCount: '观察词 {count} 个',
      hardBlockedKeywords: '高置信度硬拦截词',
      hardBlockedKeywordsPlaceholder: '每行一个明确行为意图组合词；建议保持少量、精确',
      hardBlockedKeywordsDescription: '仅匹配来源明确的当轮真实用户输入；工具、系统、环境、审批及 Responses 整段字符串不参与硬拦截。',
      hardBlockedKeywordCount: '硬拦截词 {count} 个',
      matchedSource: '命中来源',
      blockedKeywordsLimit: '最多保存 {max} 个关键词，单个长度不超过 200 个字符；重复项会自动去重。',
      keywordBlockingMode: '审计策略',
      keywordModeKeywordAndApi: '关键词 + API',
      keywordModeKeywordAndApiDesc: '命中高置信度词直接拦截；未命中时再调用上游审计接口。',
      keywordModeKeywordOnly: '仅关键词',
      keywordModeKeywordOnlyDesc: '来源明确的用户输入只用高置信度词判断；Responses 字符串和无角色输入仍交给审核 API。',
      keywordModeKeywordOnlyNotice: '当前为「仅关键词」策略；无法明确归因的 Responses 输入仍会调用审核 API。',
      keywordModeApiOnly: '仅 API',
      keywordModeApiOnlyDesc: '只调用上游审计接口判断；观察词仍记录，高置信度词不执行硬拦截。',
      keywordModeApiOnlyNotice: '当前为「仅 API」策略：高置信度词不硬拦截，请求交给上游审计接口判断。',
      overview: {
        status: '运行状态',
        enabled: '已启用',
        disabled: '未启用',
