export default {
    ...
    riskControl: {
      ...
      blockedKeywords: '观察关键词库（宽泛）',
      blockedKeywordsPlaceholder: '每行输入一个观察词；现有宽泛词库可原样保留',
      blockedKeywordsDescription: '匹配忽略大小写；命中只记为观察事件，不会拦截、计入违规、发信或封号。',
      ...
      keywordModeKeywordOnlyDesc: '只用高置信度词判断，未命中即放行，不调用上游审计接口。',
      keywordModeKeywordOnlyNotice: '当前为「仅关键词」策略：未命中高置信度词的请求将直接放行。',
