  ...
  keys: {
    ...
    useKeyModal: {
      ...
      grok: {
        ...
        note:
          'Export GROK_MODELS_BASE_URL and XAI_API_KEY, save the full config.toml (endpoints/auth/models/session/features) as ~/.grok/config.toml, run grok inspect, then /model grok-4.6 (or grok-build-0.1 for coding).',
        noteWindows:
          'Set GROK_MODELS_BASE_URL and XAI_API_KEY, save the full config.toml as %USERPROFILE%\\.grok\\config.toml, run grok inspect, then /model grok-4.6 (or grok-build-0.1 for coding).',
        claudeNote:
    ...
    quotaAmount: 'Quota Amount ({symbol})',
    ...
    resetQuotaConfirmMessage: 'Are you sure you want to reset the used quota ({symbol}{used}) for key "{name}" to 0? This action cannot be undone.',
