<template>
  <div class="group/usage relative text-sm">
    <div class="flex items-center gap-1.5">
      ...
      <span class="font-medium text-gray-900 dark:text-white">{{ billingDisplay.symbol }}{{ today.toFixed(4) }}</span>
      <Icon
        v-if="hasBreakdown"
        name="infoCircle"
        size="xs"
    ...
    <div class="mt-0.5 flex items-center gap-1.5">
      ...
      <span class="font-medium text-gray-900 dark:text-white">{{ billingDisplay.symbol }}{{ total.toFixed(4) }}</span>
    </div>
    ...
    >
      ...
      >
        ...
        <span class="font-mono">
          {{ billingDisplay.symbol }}{{ item.today_actual_cost.toFixed(4) }}
          <span class="opacity-50">/</span>
          {{ billingDisplay.symbol }}{{ item.total_actual_cost.toFixed(4) }}
        </span>
      </div>
    </div>
  </div>
