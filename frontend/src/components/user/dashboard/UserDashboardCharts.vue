<template>
  <div class="space-y-6">
    <!-- Date Range Filter -->
    <div class="card dashboard-filter-bar p-4">
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
      ...
      <div class="card dashboard-chart-panel relative overflow-hidden p-4">
            ...
            <table class="w-full text-xs">
              ...
              <tbody>
                <tr v-for="model in models" :key="model.model" class="border-t border-gray-100 dark:border-dark-700">
                  ...
                  <td class="py-1.5 text-right text-green-600 dark:text-green-400">{{ billingDisplay.symbol }}{{ formatCost(model.actual_cost) }}</td>
                  <td class="py-1.5 text-right text-gray-400 dark:text-gray-500">{{ billingDisplay.symbol }}{{ formatCost(model.cost) }}</td>
                </tr>
              </tbody>
            </table>
          </div>
...
const modelData = computed(() => !props.models?.length ? null : {
  ...
  datasets: [{
    ...
    backgroundColor: ['#e21d25', '#0f9f58', '#ffd628', '#087b83', '#111111', '#a50e15', '#79e6ec', '#6d635a']
  }]
})
