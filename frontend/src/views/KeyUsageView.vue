  ...
  if (data.mode === 'quota_limited') {
    ...
    if (data.subscription) {
      ...
      if (sub.daily_limit_usd > 0) {
        ...
        rows.push({
          iconBg: 'bg-primary-500/10', iconColor: 'text-primary-500', iconSvg: ICON_DOLLAR,
          label: `${t('keyUsage.usedQuota')} (${t('keyUsage.windowDay')})`, value: `${usd(sub.daily_usage_usd)} / ${usd(sub.daily_limit_usd)}`, valueClass: getUsageColor(pct),
        })
      }
      if (sub.weekly_limit_usd > 0) {
        ...
        rows.push({
          ...
          label: `${t('keyUsage.usedQuota')} (${t('keyUsage.windowWeek')})`, value: `${usd(sub.weekly_usage_usd)} / ${usd(sub.weekly_limit_usd)}`, valueClass: getUsageColor(pct),
        })
      }
      if (sub.monthly_limit_usd > 0) {
        ...
        rows.push({
          ...
          label: `${t('keyUsage.usedQuota')} (${t('keyUsage.windowMonth')})`, value: `${usd(sub.monthly_usage_usd)} / ${usd(sub.monthly_limit_usd)}`, valueClass: getUsageColor(pct),
        })
      }
      if (sub.expires_at) {
        rows.push({
...
function formatDate(iso: string | null | undefined): string {
  ...
  const loc = toIntlLocale(locale.value)
