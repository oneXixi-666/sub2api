<template>
  <div class="promo-payment-standalone flex min-h-screen items-center justify-center px-4">
    <div class="promo-payment-receipt w-full max-w-md space-y-6">
      <!-- Loading -->
        ...
        <div v-if="order" class="card p-5">
          <div class="space-y-3 text-sm">
            ...
            <div v-if="hasAmountFields(order) && order.amount !== order.pay_amount" class="flex justify-between">
              ...
              <span class="font-medium text-gray-900 dark:text-white">{{ order.order_type === 'balance' ? formatBillingAmount(order.amount) : formatGatewayAmount(order.amount) }}</span>
            </div>
