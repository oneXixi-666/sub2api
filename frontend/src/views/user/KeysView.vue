<template>
  <AppLayout>
          ...
          <template #cell-group="{ row }">
            <div class="relative">
              <button
                ...
                class="group -mx-2 -my-1 flex cursor-pointer items-center gap-2 rounded-lg px-2 py-1 transition-all duration-200 hover:bg-gray-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-500/60 dark:hover:bg-dark-700"
              ...
              >
                ...
                <svg
                  class="h-3.5 w-3.5 text-gray-400 opacity-60 transition-opacity group-hover:opacity-100"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                  stroke-width="2"
            ...
            <div class="text-sm">
              <div class="flex items-center gap-1.5">
                ...
                <span class="font-medium text-gray-900 dark:text-white">
                  {{ billingDisplay.symbol }}{{ (usageStats[row.id]?.today_actual_cost ?? 0).toFixed(4) }}
                </span>
              </div>
              <div class="mt-0.5 flex items-center gap-1.5">
                <span class="text-gray-500 dark:text-gray-400">{{ t('keys.total') }}:</span>
                <span class="font-medium text-gray-900 dark:text-white">
                  {{ billingDisplay.symbol }}{{ (usageStats[row.id]?.total_actual_cost ?? 0).toFixed(4) }}
                </span>
              </div>
              ...
              <div v-if="row.quota > 0" class="mt-1.5">
                <div class="flex items-center gap-1.5">
                  ...
                  <span :class="[
                    ...
                    {{ billingDisplay.symbol }}{{ row.quota_used?.toFixed(2) || '0.00' }} / {{ billingDisplay.symbol }}{{ row.quota?.toFixed(2) }}
                  </span>
                </div>
              ...
              <div v-if="row.rate_limit_5h > 0">
                <div class="flex items-center justify-between text-xs">
                  ...
                  <span :class="[
                    ...
                    {{ billingDisplay.symbol }}{{ row.usage_5h?.toFixed(2) || '0.00' }}/{{ billingDisplay.symbol }}{{ row.rate_limit_5h?.toFixed(2) }}
                  </span>
                </div>
              ...
              <div v-if="row.rate_limit_1d > 0">
                <div class="flex items-center justify-between text-xs">
                  ...
                  <span :class="[
                    ...
                    {{ billingDisplay.symbol }}{{ row.usage_1d?.toFixed(2) || '0.00' }}/{{ billingDisplay.symbol }}{{ row.rate_limit_1d?.toFixed(2) }}
                  </span>
                </div>
                <div class="h-1 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-dark-600">
                  <div
              ...
              <div v-if="row.rate_limit_7d > 0">
                <div class="flex items-center justify-between text-xs">
                  ...
                  <span :class="[
                    ...
                    {{ billingDisplay.symbol }}{{ row.usage_7d?.toFixed(2) || '0.00' }}/{{ billingDisplay.symbol }}{{ row.rate_limit_7d?.toFixed(2) }}
                  </span>
                </div>
                <div class="h-1 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-dark-600">
                  <div
      ...
      <form id="key-form" @submit.prevent="handleSubmit" class="space-y-5">
        ...
        <div>
          ...
          <button
            ...
            class="flex min-h-11 w-full items-center justify-between gap-3 rounded-xl border border-gray-200 bg-white px-3.5 py-2 text-left transition hover:border-primary-400 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-500/50 disabled:cursor-not-allowed disabled:opacity-60 dark:border-dark-600 dark:bg-dark-800 dark:hover:border-primary-500"
            :disabled="groups.length === 0"
            @click="openFormGroupSelector"
          >
            <GroupBadge
              v-if="selectedFormGroup"
              :name="selectedFormGroup.label"
              :platform="selectedFormGroup.platform"
              :subscription-type="selectedFormGroup.subscriptionType"
              :rate-multiplier="selectedFormGroup.rate"
              :user-rate-multiplier="selectedFormGroup.userRate"
              :peak-rate-enabled="selectedFormGroup.peakRateEnabled"
              :peak-start="selectedFormGroup.peakStart"
              :peak-end="selectedFormGroup.peakEnd"
              :peak-rate-multiplier="selectedFormGroup.peakRateMultiplier"
            />
            <span v-else class="text-sm text-gray-400 dark:text-dark-500">{{ t('keys.selectGroup') }}</span>
        ...
        <div class="space-y-3">
          ...
          <div class="space-y-4">
            <div>
              <div class="relative">
                <span class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500">{{ billingDisplay.symbol }}</span>
                <input
                  ...
                  min="0"
                  class="input pl-7"
                  :placeholder="t('keys.quotaAmountPlaceholder', { currency: billingDisplay.currency })"
                />
              </div>
            ...
            <div v-if="showEditModal && selectedKey && selectedKey.quota > 0">
              ...
              <div class="flex items-center gap-2">
                <div class="flex-1 rounded-lg bg-gray-100 px-3 py-2 dark:bg-dark-700">
                  <span class="font-medium text-gray-900 dark:text-white">
                    {{ billingDisplay.symbol }}{{ selectedKey.quota_used?.toFixed(4) || '0.0000' }}
                  </span>
                  <span class="mx-2 text-gray-400">/</span>
                  <span class="text-gray-500 dark:text-gray-400">
                    {{ billingDisplay.symbol }}{{ selectedKey.quota?.toFixed(2) || '0.00' }}
                  </span>
                </div>
                <button
                  type="button"
        ...
        <div class="space-y-3">
          ...
          <div v-if="formData.enable_rate_limit" class="space-y-4 pt-2">
            ...
            <!-- 5-Hour Limit -->
            <div>
              <label class="input-label">{{ t('keys.rateLimit5h', { currency: billingDisplay.currency }) }}</label>
              <div class="relative">
                <span class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500">{{ billingDisplay.symbol }}</span>
                <input
                ...
                <div class="flex items-center gap-2">
                  <div class="flex-1 rounded-lg bg-gray-100 px-3 py-2 dark:bg-dark-700 text-sm">
                    <span :class="[
                      ...
                      {{ billingDisplay.symbol }}{{ selectedKey.usage_5h?.toFixed(4) || '0.0000' }}
                    </span>
                    <span class="mx-2 text-gray-400">/</span>
                    <span class="text-gray-500 dark:text-gray-400">
                      {{ billingDisplay.symbol }}{{ selectedKey.rate_limit_5h?.toFixed(2) || '0.00' }}
                    </span>
                  </div>
                </div>
                <div class="mt-1 h-1.5 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-dark-600">
            ...
            <!-- Daily Limit -->
            <div>
              <label class="input-label">{{ t('keys.rateLimit1d', { currency: billingDisplay.currency }) }}</label>
              <div class="relative">
                <span class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500">{{ billingDisplay.symbol }}</span>
                <input
                ...
                <div class="flex items-center gap-2">
                  <div class="flex-1 rounded-lg bg-gray-100 px-3 py-2 dark:bg-dark-700 text-sm">
                    <span :class="[
                      ...
                      {{ billingDisplay.symbol }}{{ selectedKey.usage_1d?.toFixed(4) || '0.0000' }}
                    </span>
                    <span class="mx-2 text-gray-400">/</span>
                    <span class="text-gray-500 dark:text-gray-400">
                      {{ billingDisplay.symbol }}{{ selectedKey.rate_limit_1d?.toFixed(2) || '0.00' }}
                    </span>
                  </div>
                </div>
                <div class="mt-1 h-1.5 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-dark-600">
            ...
            <!-- 7-Day Limit -->
            <div>
              <label class="input-label">{{ t('keys.rateLimit7d', { currency: billingDisplay.currency }) }}</label>
              <div class="relative">
                <span class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500">{{ billingDisplay.symbol }}</span>
                <input
                ...
                <div class="flex items-center gap-2">
                  <div class="flex-1 rounded-lg bg-gray-100 px-3 py-2 dark:bg-dark-700 text-sm">
                    <span :class="[
                      ...
                      {{ billingDisplay.symbol }}{{ selectedKey.usage_7d?.toFixed(4) || '0.0000' }}
                    </span>
                    <span class="mx-2 text-gray-400">/</span>
                    <span class="text-gray-500 dark:text-gray-400">
                      {{ billingDisplay.symbol }}{{ selectedKey.rate_limit_7d?.toFixed(2) || '0.00' }}
                    </span>
                  </div>
                </div>
                <div class="mt-1 h-1.5 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-dark-600">
    ...
    <ConfirmDialog
      ...
      :message="t('keys.resetQuotaConfirmMessage', { name: selectedKey?.name, used: selectedKey?.quota_used?.toFixed(4), symbol: billingDisplay.symbol })"
    ...
    <!-- Shared group picker for API key creation, editing, and inline reassignment -->
    <GroupSelectorModal
      :show="showGroupSelectorModal"
      :groups="groups"
...
import BulkEditKeysModal from '@/components/keys/BulkEditKeysModal.vue'
	...
	import GroupSelectorModal from '@/components/keys/GroupSelectorModal.vue'
...
type GroupSelectorTarget = 'form' | 'key'
const groupSelectorTarget = ref<GroupSelectorTarget | null>(null)
...
const selectedKeyForGroup = computed(() => {
  if (groupSelectorTarget.value !== 'key' || groupSelectorKeyId.value === null) return null
...
const groupPickerValue = computed(() => {
  if (groupSelectorTarget.value === 'form') return formData.value.group_id
  return selectedKeyForGroup.value?.group_id ?? null
})
...
const openFormGroupSelector = () => {
  groupSelectorTarget.value = 'form'
  groupSelectorKeyId.value = null
  showGroupSelectorModal.value = true
}

const openGroupSelector = (key: ApiKey) => {
  groupSelectorTarget.value = 'key'
...
const handleGroupSelected = async (newGroupId: number | null) => {
  if (groupSelectorTarget.value === 'form') {
  ...
  closeGroupSelector()
  if (key) await changeGroup(key, newGroupId)
}

const closeGroupSelector = () => {
  showGroupSelectorModal.value = false
  groupSelectorTarget.value = null
  groupSelectorKeyId.value = null
}
