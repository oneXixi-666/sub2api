<template>
  <div class="promo-payment-standalone flex min-h-screen items-center justify-center p-4">
    <div
      class="card promo-payment-receipt w-full max-w-md space-y-4 p-6"
    >
      <!-- Amount + Order ID -->
      <div v-if="amount" class="text-center">
        <p class="text-3xl font-bold" :style="{ color: methodColor }">{{ billingDisplay.symbol }}{{ amount }}</p>
